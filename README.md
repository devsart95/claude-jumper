...
out at 64 pt, so each one leaves at least 0.33 s to time the jump. The run speeds up for a full minute and obstacles are
placed by distance with that in mind, so even a last-moment jump lands at least 0.2 s before you have to jump the next one.
`swift test` checks all of it.
